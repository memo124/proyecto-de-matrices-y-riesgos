...
require('../models/kt_usersapp.php');
...
$usuario = new UserApp;
if ( $dataUsuario = $usuario->readUser() ) {
    ...
    $pdf->Cell(93, 10, utf8_decode('Nivel'), 1, 0, 'C', 1);
    ...
    foreach ( $dataUsuario as $rowUsuario ) {
        ...
        $pdf->Cell(93, 10, utf8_decode($rowUsuario['user_id']), 1, 0, 'C');
        $pdf->Cell(93, 10, utf8_decode($rowUsuario['level']), 1, 0, 'C');
        $pdf->Cell(37.3, 10, utf8_decode(' '), 0, 1, 'C');
    }
} else {
