...
class Validator
{
    ...
    public function validatePassword($value, $minimum, $maximum)
    {
        if(preg_match('/[a-zA-Z]{'.$minimum.','.$maximum.'}/', $value)){
            if (preg_match('/[0-9]{'.$minimum.','.$maximum.'}/', $value)) {
                if(preg_match('/[ñÑáÁéÉíÍóÓúÚ]{'.$minimum.','.$maximum.'}/', $value)){
                    if ( strlen($value) > 8 && strlen($value ) < 27 ) {
                        return true;
                    } else {
                        return false;
                    }
